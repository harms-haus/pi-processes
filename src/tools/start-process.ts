...
import { StartProcessSchema, DEFAULT_START_DELAY } from "../types.js";

export function createStartProcessTool(getManager: () => ProcessManager): ToolDefinition<typeof StartProcessSchema, StartupResult> {
  return {
    name: "start_process",
    label: "Start Process",
    description: "Start a long-running process (e.g., debug server, API server). Waits for startup logs to settle before returning.",
