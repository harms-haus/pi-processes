...
export function createListProcessesTool(getManager: () => ProcessManager): ToolDefinition<typeof ListProcessesSchema, ListProcessesResult> {
  return {
    name: "list_processes",
    label: "List Processes",
    description: "List all active managed processes with their status.",
    promptSnippet: "List active managed processes",
    promptGuidelines: ["Use list_processes to see all running managed processes."],
    ...
    renderCall(_args, theme) {
      return new Text(theme.fg("toolTitle", theme.bold("list_processes")), 0, 0);
    },
    renderResult(result, _options, theme) {
      ...
      container.addChild(new Text(
        ...
        theme.fg("dim", ` — ${details.count} process${details.count !== 1 ? "es" : ""}`),
        0, 0
      ));
      if (details.count > 0) {
