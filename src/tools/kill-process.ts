...
export function createKillProcessTool(
  ...
  return {
    name: "kill_process",
    label: "Kill Process",
    description: "Kill a running managed process by name. Returns total runtime.",
    ...
    renderCall(args, theme) {
      return new Text(
        theme.fg("warning", theme.bold("kill_process ")) + theme.fg("accent", args.name),
        0,
        0,
      );
    },
      ...
      container.addChild(
        new Text(
          theme.fg("error", theme.bold(`✗ ${details.name}`)) +
            theme.fg("dim", ` PID ${details.pid} | runtime ${totalRuntimeSec.toFixed(1)}s`),
          0,
          0,
        ),
      );
