...
export class LogDialog {
	...
	constructor(
		...
		private onDone: (
			result: { selectedLogs: LogEntry[]; processName: string } | null,
		) => void,
	) {}
	...
	private renderContent(width: number): string[] {
		...
		if (this.viewportOffset > 0) {
			...
			lines.push(
				truncateToWidth(
					this.theme.fg("dim", `↑ ${above} more above`),
					width,
				),
			);
		}
		...
		const endIdx = Math.min(
			this.viewportOffset + viewportHeight,
			logs.length,
		);
		...
		if (endIdx < logs.length) {
			...
			lines.push(
				truncateToWidth(
					this.theme.fg("dim", `↓ ${below} more below`),
					width,
				),
			);
		}
	...
	private renderDiagnostics(width: number): string {
		...
		if (logs.length > 0) {
			...
			const endIdx = Math.min(
				this.viewportOffset + effectiveHeight,
				logs.length,
			);
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, Key.shift("tab"))) {
			if (this.processes.length > 1) {
				this.activeTabIndex =
					(this.activeTabIndex - 1 + this.processes.length) %
	...
	private moveSelection(delta: number, isMultiSelect: boolean): void {
		...
		this.selectedIndex = Math.max(
			0,
			Math.min(logs.length - 1, this.selectedIndex + delta),
		);
	}
