 ...
 */
async function startAndResolve(pm: ProcessManager, name: string, command: string, startDelay = 1) {
...
describe('ProcessManager', () => {
  ...
  describe('start()', () => {
    ...
    it('throws if name already exists', async () => {
      ...
      await expect(pm.start('test', 'echo again', 1)).rejects.toThrow(/already exists/i);
    });
    ...
    it('throws if MAX_PROCESSES reached', async () => {
      ...
      mockSpawn.mockReturnValue(createMockChildProcess());
      await expect(pm.start('overflow', 'echo x', 1)).rejects.toThrow(/maximum/i);
    });
  ...
  describe('kill()', () => {
    ...
    it('prevents concurrent kill calls on the same process', async () => {
      ...
      await expect(pm.kill('test')).rejects.toThrow(/already being killed/);
  ...
  describe('restart()', () => {
    ...
    it('uses new command if provided', async () => {
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith('echo v2', [], expect.anything());
    });
    ...
    it('uses previous command if no command provided', async () => {
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith('echo original', [], expect.anything());
    });
