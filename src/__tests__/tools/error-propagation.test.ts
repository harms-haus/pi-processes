...
import { createMockManager, executeTool } from "../helpers/index.js";
...
describe("tool error propagation", () => {
  ...
  it("kill-process propagates error from manager.kill", async () => {
    ...
    await expect(executeTool(tool, "call-2", { name: "missing" })).rejects.toThrow("not found");
  });
  ...
  it("restart-process propagates error from manager.restart", async () => {
    ...
    await expect(executeTool(tool, "call-3", { name: "broken" } as any)).rejects.toThrow(
  ...
  it("process-logs propagates error from manager.getLogs", async () => {
    ...
    await expect(executeTool(tool, "call-4", { name: "missing" })).rejects.toThrow("not found");
  });
  ...
  it("list-processes propagates error from manager.list", async () => {
    ...
    await expect(executeTool(tool, "call-5", {})).rejects.toThrow("internal error");
  });
});
