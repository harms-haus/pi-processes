 ...
 * - has → returns false
 * - size → returns 0
 * - killAll, shutdown → resolve undefined
...
export function createMockManager(
	...
	const start = vi.fn<(...args: any[]) => Promise<StartupResult>>().mockResolvedValue(
		defaultStartupResult,
	);
	const kill = vi.fn<(...args: any[]) => Promise<KillResult>>().mockResolvedValue(
		defaultKillResult,
	);
	const restart = vi.fn<(...args: any[]) => Promise<StartupResult>>().mockResolvedValue(
		defaultStartupResult,
	);
	...
	const killAll = vi.fn<() => Promise<void>>().mockResolvedValue(undefined);
